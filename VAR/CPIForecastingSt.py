...
# Stochastic Forecast
num_simulations = 1000000  # Number of stochastic simulations
simulated_forecasts = []

for _ in range(num_simulations):
    fc = model_fitted.forecast(y=forecast_input, steps=nobs)
    noise = np.random.normal(0, 1, fc.shape)  # Additive noise
    simulated_forecast = fc + noise
    simulated_forecasts.append(simulated_forecast)

# Convert the forecasts to DataFrame and average them
simulated_forecasts = np.array(simulated_forecasts)
mean_forecast = simulated_forecasts.mean(axis=0)
df_forecast = pd.DataFrame(mean_forecast, index=df.index[-nobs:], columns=df.columns + '_d')
